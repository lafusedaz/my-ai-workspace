...
if "tasks" not in st.session_state:
    st.session_state.tasks = [{"id": 1, "target": "ญข-9999 (Sylphy)", "detail": "แปลงโฉมเป็น Sentra ตรงรุ่น + พ่นสีน้ำเงินรอบคัน 2K", "user": "staff1", "status": "กำลังทำ", "update_time": "2026-06-25 14:00", "notes": ""}]
...
if "user_role" not in st.session_state:
    ...
    with col_hr:
        if st.button("ออกจากระบบ 🏃‍♂️", use_container_width=True):
            del st.session_state.user_role, st.session_state.username; st.rerun()
    st.divider()
    ...
    if menu == "👥 จัดการระบบ USER" and role == "Manager":
        ...
        with st.expander("➕ เปิด Job งานใหม่ (กรอกข้อมูล 3 ช่อง)"):
            ...
            if st.button("บันทึกเปิด Job") and t_target and t_detail:
                st.session_state.tasks.append({"id": len(st.session_state.tasks)+1, "target": t_target, "detail": t_detail, "user": current_user, "status": "กำลังทำ", "update_time": datetime.datetime.now().strftime("%Y-%m-%d %H:%M"), "notes": ""}); st.rerun()
        ...
        for task in (st.session_state.tasks if role == "Manager" else [t for t in st.session_state.tasks if t["user"] == current_user]):
            with st.container(border=True):
                ...
                c2.markdown(f"**🔧 รายละเอียด:**\n{task['detail']}\n" + (f"📝 *บันทึก:* {task['notes']}" if task['notes'] else ""))
        ...
        if user_msg:
            ...
            st.session_state.tasks.append({"id": len(st.session_state.tasks)+1, "target": "🎯 แผนงานจากระบบ AI", "detail": f"ระดมสมอง: {user_msg}", "user": "AI_Automation", "status": "กำลังทำ", "update_time": datetime.datetime.now().strftime("%Y-%m-%d %H:%M"), "notes": ""})
            st.rerun()
